...
import { Plus, Pencil, Trash2, Check, X, PackageCheck, PackageX } from 'lucide-react'
  ...
  return (
    <div className="space-y-5">
      ...
      {showAdd && (
        <div className="bg-white rounded-xl border border-[#e8e2d9] p-5 space-y-3">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <input value={newProduct.slug} onChange={e => setNewProduct(p => ({ ...p, slug: e.target.value }))} placeholder="slug (e.g. baby-lotion)" className="px-3 py-2 rounded-lg border border-[#e8e2d9] text-sm font-mono focus:outline-none focus:ring-2 focus:ring-[#b0925e]/30" />
      ...
      {grouped.map(({ category, label, items }) => (
        <div key={category}>
          <h2 className="text-sm font-semibold text-[#9e9791] uppercase tracking-wider mb-2">{label} ({items.length})</h2>
          <div className="bg-white rounded-xl border border-[#e8e2d9] divide-y divide-[#e8e2d9]">
                ...
                {editingId === p.id ? (
                  <div className="space-y-2">
                    <div className="grid sm:grid-cols-3 gap-2">
                      <input value={editData.name || ''} onChange={e => setEditData(d => ({ ...d, name: e.target.value }))} className="px-2 py-1.5 rounded border border-[#e8e2d9] text-sm focus:outline-none focus:ring-1 focus:ring-[#b0925e]/30" />
                      <input type="number" value={editData.price || ''} onChange={e => setEditData(d => ({ ...d, price: Number(e.target.value) }))} className="px-2 py-1.5 rounded border border-[#e8e2d9] text-sm focus:outline-none focus:ring-1 focus:ring-[#b0925e]/30" />
                      <select value={editData.category || ''} onChange={e => setEditData(d => ({ ...d, category: e.target.value }))} className="px-2 py-1.5 rounded border border-[#e8e2d9] text-sm bg-white focus:outline-none focus:ring-1 focus:ring-[#b0925e]/30">
                        {CATEGORIES.map(c => <option key={c} value={c}>{c.replace('-', ' ')}</option>)}
                      </select>
                    </div>
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {!p.in_stock && (
                            <span className="text-xs px-1.5 py-0.5 rounded bg-red-50 text-red-600 font-medium">Out of stock</span>
                          )}
                        </div>
                        <p className="text-xs text-[#9e9791] truncate">{p.description}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="font-medium text-sm text-[#2c2825]">GH₵ {p.price}</span>
                      <button onClick={() => toggleStock(p)} title={p.in_stock ? 'Mark out of stock' : 'Mark in stock'} className={`p-1.5 rounded transition-colors ${p.in_stock ? 'text-green-600 hover:bg-green-50' : 'text-red-400 hover:bg-red-50'}`}>
                      ...
                      {deleting === p.id ? (
                        ...
                        <button onClick={() => setDeleting(p.id)} className="p-1.5 rounded text-[#9e9791] hover:bg-red-50 hover:text-red-600"><Trash2 size={16} /></button>
                      )}
                    </div>
                  </div>
                )}
            ...
            {items.length === 0 && (
              <div className="px-4 py-8 text-center text-sm text-[#9e9791]">No products in this category</div>
            )}
          </div>
        </div>
      ))}
